...
MOEX Select имеет два режима:

1. `Новичок` - анкета с целью, риском, горизонтом, суммой и классами инструментов. Backend применяет recommendation engine и возвращает объяснимую подборку.
2. `Опытный` - расширенный поиск по данным MOEX ISS. Пользователь задает конкретные фильтры, сортировку и работает с пагинацией по шесть карточек.
...
- объем, оборот, волатильность, капитализацию, тип опциона и strike price в расширенном поиске;
...
- использует `GET /api/instruments` как основной экран расширенного поиска;
- позволяет переключаться между режимами `Новичок` и `Опытный`;
- показывает форму сверху или слева и результаты ниже или справа в зависимости от ширины экрана;
- выводит результаты поиска по шесть карточек на страницу;
